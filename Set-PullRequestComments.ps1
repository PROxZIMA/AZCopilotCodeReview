function Set-PullRequestComments {
    ...
    $base64Pat = [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes(":$AzureDevOpsPat"))
    $headers = @{
    ...
    try {
        ...
        throw "Failed to authenticate with Azure DevOps. Verify AZURE_DEVOPS_PAT has 'Code: Read' and 'Pull Request Threads: Read & Write' permissions. Error: $_"
    }
    ...
    try {
        $reviewsObject = $Reviews | ConvertFrom-Json -ErrorAction Stop
    } catch {
        Write-Warning "Failed to parse Copilot review response as JSON: $_"
        return
    }
